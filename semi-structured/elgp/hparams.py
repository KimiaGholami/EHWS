...
Values for OPT-125M and OPT-1.3B are taken from ELSA's own published
hyperparameter table, since Phase 2 uses the same global ADMM structure
ELSA itself uses. A few notes on how the table was filled in for points
ELSA didn't report directly:
...
- HGRN-1.3B has no published reference point at all (it isn't one of
  ELSA's models). We use OPT-1.3B's row as the closest available
  analogue by parameter count -- an approximation, not a measured value.
"""
